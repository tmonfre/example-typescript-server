...
function requireAdminOrAuth(req: Request, res: Response, next: NextFunction): void {
  passport.authenticate('jwt', { session: false }, (err, user) => {
    ...
    // only allow to continue if user making request on self or user is admin
    if (!((userId && userId === user.id) || (email && email === user.email) || user.isAdmin)) {
...
export default requireAdminOrAuth;
